...
// NOTE: Some properties on the User Schema are encrypted.
// these properties have setters and getters that encrypt
// the field before it is written to the DB and decrypt it 
// before it is returned from the DB these fields must be of type
// String and can be identified becuase of their set and get properties
// ex. 
// firstName: {type: String, set:encrypt, get: decrypt},
// to query for these properties you will need to encrypt the
// query value before querying
// ex.
// User.find({firstName: encrypt("+19175657456")})
// The models export the encryption helpers so you 
// will be able to do this anywhere you have access to the
// models
...
function BaseSchema(){
  ...
  this.add({
    ...
    email: { type: String, lowercase: true, set:encrypt, get: decrypt},
    gender: {type: String},
...
BaseUserSchema.set('toObject', {getters: true});
BaseUserSchema.set('toJSON', {getters: true});



// sends sms to a user
BaseUserSchema.methods.sendSms = function (msg, mediaUrl, statusCallback) {
  var self = this;
  if(!this.phoneNumber) {
    return Promise.reject(new Error(self.name + " has no phone number"));
  }
  var sms = new Sms(self.phoneNumber, msg, mediaUrl, statusCallback)
  return sms.send();
}

// finds a user by their unique phone number
BaseUserSchema.statics.findByPhoneNumber = function(phoneNumber){
  return this.findOne({phoneNumber: encrypt(phoneNumber)});
};
...
var WaitListUserSchema = new BaseSchema({
  ...
  lastName: {type: String, set:encrypt, get: decrypt},
  email: { type: String, lowercase: true, set:encrypt, get: decrypt},
  zipCode: { type: String, lowercase: true, set:encrypt, get: decrypt},
  phoneNumber: { type: String, lowercase: true, set:encrypt, get: decrypt},
...
var UserSchema = new BaseSchema({
  ...
  hasVerifiedPhone: {type: Boolean},
  isPasswordSet: {type: Boolean, default: false},
  birthday: {type: Date},
  zipCode: {type: String, set:encrypt, get:decrypt},
  cards: [{type: mongoose.Schema.Types.ObjectId, ref: 'InfoNudge'}],
  phoneNumber: {type: String, set:encrypt, get: decrypt},
  twilioFromNumber: {type: String,  minlength: 12, maxlength: 12},
  ...
  referralCodeUsed: {type: mongoose.Schema.Types.ObjectId, ref: 'ReferralCode'},
  yodlee_username: {type: String, set:encrypt, get:decrypt},
  ...
  phoneContacts: {type: Array}, // this has been deprecated, phone contacts now live in their own collection
...
// },
  ...
  primaryChecking: {type: mongoose.Schema.Types.ObjectId, ref: 'PlaidAccount'},
  primaryCheckingInfo: {
    lastAbove: {
    ...
    below: {type: Boolean},
    belowDates: {type: Array },
    sentDate: {type: Date},
    canSendAgain: {type: Boolean}, 
    itsmFaked: {type: Boolean}
  }
...
UserSchema
  .pre('save', function(next){
    if (!this.isNew) return next();
    if (!this.phoneNumber || this.phoneNumber !== "+10987654321") return next();
    this.constructor.remove({phoneNumber: encrypt("+10987654321")})
    .then(function(){
      next();
    })
    .then(null, function(err){
      console.log("Something went oh so wrong while removing users with phoneNumber = +10987654321 :", err)
      next(err);
    })
  })
