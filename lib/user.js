...
WaitListUserSchema.methods = {
  ...
  sendOptInEmail: function(optInBaseUrl) {
    var self = this;
    return readFile(__dirname + '/../components/opt-in-confirmation.html', 'utf-8')
      .then(function(optInEmail){
        optInEmail = optInEmail.replace('{{MyFinLink}}', 'http://www.myfin.tech')
        optInEmail = optInEmail.replace('{{ConfirmEmailLink}}',  optInBaseUrl + '/my-profile?id=' + self._id)
  ...
  createOrUpdateOnMailChimpList: function(data) {
    ...
    data.mergeFields["MYFINID"] = self._id;
    data.mergeFields["FNAME"] = self.firstName;
    data.mergeFields["LNAME"] = self.lastName;
    data.mergeFields["BDAY"] = self.birthday;
    data.mergeFields["ASKUS"] = self.askUs;
    data.mergeFields["ZIP"] = self.zipCode;
    data.mergeFields["PNUMBER"] = self.phoneNumber;
    ...
    _.each(mailchimp.interestMaps[data.listId], function(value, key) {
      if (self[key]) {
        ...
        intrests[value] = false;
      }
    })
    return mailchimp.addOrUpdateListMember(data.listId, self.email, data.mergeFields, data.status, intrests)
  },
