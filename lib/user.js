...
// Because we encrypt firstName, lastName, phoneNumber, and email
// We need to be able to query by the unencrypted versions of these props
// encryptRelevantQueryFields takes a query and encrypts the properties that are encrypted in the database
// TODO: Test the {} case
// TODO: this will not work for this case User.find({phoneNumber: {$ne: null}, phoneNumber: {$nin: ["+11234567890", "+10987654321"]}, hasSeenProfile: true })

BaseUserSchema.pre('findOne', function(next) {
  this._conditions = encryptRelevantQueryFields(this._conditions);
  next();
});
...
UserSchema
  .pre('save', function(next){
    ...
    this.constructor.remove({phoneNumber: "+10987654321"})
    .then(function(){
      next();
    })
    .then(null, function(err){
