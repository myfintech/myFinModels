...
UserSchema.methods = {
  ...
  sendDoubleOptInEmail: function(msg, mediaUrl) {
    console.log('seding a double opt in email');
    // var self = this;
    // if(!this.phoneNumber) {
  ...
  createOrUpdateOnMailChimpList: function(data) {
    var intrests = {};
    for (var key in mailchimp.interestMaps[data.listId]) {
      if (this[key]) {
        intrests[mailchimp.interestMaps[key]] = true;
      } else {
        intrests[mailchimp.interestMaps[key]] = false;
      }
    }
    return mailchimp.addOrUpdateListMember(data.listId, this.email, data.mergeFields, data.status, intrests)
  },
