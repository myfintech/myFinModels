...
// gets two lists of SMS messages on from us to a user and one from a user to us
BaseUserSchema.methods.getSms = function () {
  var self = this;
  if(!this.phoneNumber) {
    return Promise.reject(new Error(self.name + " has no phone number"));
  }
  ...
  return Promise.props({
    toUser: sms.getMessageList({to: self.phoneNumber}),
    fromUser: sms.getMessageList({from: self.phoneNumber})
  })
}
