...
BaseTransactionSchema.methods.isTransferFromBrokerageAccount = function(){
  return /brokerage/i.test(this.name);  
};
...
BaseTransactionSchema.methods.inverseDocBelongsToInstitutionWithoutTransactions = function(user){
  ...
  if (this.isDebit() && this.targetId === "31575") return true; 
...
BaseTransactionSchema.methods.isMovingMoneyToOrFromWealthfront = function(){
  return /apex clearing ach/i.test(this.name); 
};
...
BaseTransactionSchema.methods.addAccountCounterpart = function(user){
  var accountIds = Object.keys(user.sourceAccountIdToAccountMyFinTypeMap); 
  var possibleAccountCounterparts = []; 
  for (var i = 0, len = accountIds.length; i < len; i++){
    if (String(user.sourceAccountIdToAccountMyFinTypeMap[accountIds[i]]) === String(this.institutionCounterpart)){
...
BaseTransactionSchema.methods.isNotAssociatedWithVenmo = function(){
  return this.category_id !== "21010001";
};
...
BaseTransactionSchema.methods.isTransferToBetterment = function(){
  return this.isDebit() && (/betterment/i.test(this.name) || this.category_id === "21010010"); 
};
BaseTransactionSchema.methods.isDebitFromCheckingThatIsTransferToPaypal = function(user){
  // if this debit is a debit that moves money into paypal, and if paypal is in the ecosystem, this is a transfer
  // if paypal is not in the ecosystem, it's spent
  return this.isDebit() && this.category_id === "21010004" && user.sourceInstitutions.indexOf("7298") > 0; 
};
...
BaseTransactionSchema.methods.markSubtypeAsTransferToInvestmentAccount = function(){
  this.subType = "investments"; 
  return this; 
};
/////////
