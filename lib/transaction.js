...
var subTypes = ['savings', 'loanPayment', 'creditCardPayment', "inflowToChecking", "unknown", "outOfNetworkTransfer", "refund", "transferToVenmo", "withdrawal", "debitSideOfCreditCardPayment", "isTransferToWealthfront", "transferToFidelity", "paycheck", "transferFromVenmo"]; 
...
BaseTransactionSchema.methods.isATMWithdrawal= function(){
  if (!this.isDebit()) return false;  
  // specifically denotes ATM withdrawal
  return this.category_id === "21012002";
};
...
BaseTransactionSchema.methods.isCashOutFromVenmo = function(){
  if (!this.isDebit()) return false; 
  return this.sourceInstitutionId === "30671" && /cashed out/i.test(this.name);   
};
...
BaseTransactionSchema.methods.isDebitFromVenmoThatIsntCashedOut = function(){
  ...
  return (this.sourceInstitutionId === "30671" || this.institution.toString() !== process.env.VENMO_ID) && this.name !== "CASHED OUT" && this.name !== "Cashed Out";   
};
  ...
  if (categoryIds.indexOf(this.category_id) > -1){
    ...
    for (var i = 0, len = keys.length; i < len; i ++){
      if (institutionIdToPlaidId[keys[i]] === this.category_id){
        ...
        // CREDITS to venmo are either "fromFriends" or "transfers"
        // but if the CREDIT has a category_id of 2101001, it's a transfer
        // if the DEBIT has a category_id of 2101001, it's a transfer 
        // if the CREDIT does NOT have a category_id of 2101001, its fromFriends
...
BaseTransactionSchema.methods.isCreditToFidelityInvestments = function(){
...
BaseTransactionSchema.methods.markSubtypeAsTransferToFidelity= function(){
  this.subType = "transferToFidelity"; 
  return this; 
};
