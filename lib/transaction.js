...
var subTypes = ['savings', 'loanPayment', 'creditCardPayment', "unknown", "outOfNetworkTransfer"]; 
...
BaseTransactionSchema.methods.isWithinRange = function(startOfRange, endOfRange){
  ...
  if (dateOfCredit.isSame(startOfRange) && this.absAmount === 1300) console.log('date of credit is same as start of range', this) 
  return ( dateOfCredit.isBefore(endOfRange) || dateOfCredit.isSame(endOfRange) ) && ( dateOfCredit.isAfter(startOfRange)  || dateOfCredit.isSame(startOfRange) );
};
...
BaseTransactionSchema.methods.marksAsInverse = function(){
...
BaseTransactionSchema.methods.isItMovingIntoLinkedAccount = function(user){
...
if (this.targetId === "31575"){

  console.log('user source institions: ', user.sourceInstitutions)
  console.log('id: ', this._id)
}
  // if this DEBIT moved into a LINKED account, mark as transfer
  if (user.sourceInstitutions.indexOf(this.targetId) > 0) return true; 
  // this user has NOT linked with the institution into which she moved the money
  
  this.subType = "outOfNetworkTransfer";
  return false;
};
...
BaseTransactionSchema.methods.markAsFromFriendsOrMade = function(){
  console.log('INSIDE MARK FROM FRIENDS OR MADE ---- > venmo id on process env???', process.env.VENMO_ID);
