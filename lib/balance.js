...
BalanceSchema.statics = {
  ...
  createBalanceFromPlaid: function(user, account){
    var displayNumber; 
    if (account.myFinType === "creditCard"){
      displayNumber = account.balance.current || account.balance.available; 
    }
    else displayNumber = typeof account.balance.available === "number" ? account.balance.available : account.balance.current; 
